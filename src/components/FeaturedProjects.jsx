...
import { Bike, Car, Package, ArrowRight, ShieldCheck, Clock, MapPin, Zap } from 'lucide-react';
  ...
  const services = [
    {
      ...
      bgStyle: 'bg-yellow-card', image: '/assets/project-superapp.png', perks: ['Fast pickups', 'Transparent fare', 'Helmet & safety'],
    ...
    },
    {
      ...
      bgStyle: 'bg-gray-card', image: '/assets/hero-phone.png', perks: ['Upfront fares', 'Auto & Cab', 'Live tracking'],
    ...
    },
    {
      ...
      bgStyle: 'bg-green-card', image: '/assets/project-leafly.png', perks: ['Doorstep pickup', 'Live tracking', 'Sender to receiver'],
    ...
    },
    {
      ...
      bgStyle: 'bg-beige-card', image: '/assets/project-superapp.png', perks: ['One app', 'Multiple ride types', 'Easy booking'],
  ...
  return (
    <section id="services-grid" className="services-showcase-section">
      <div className="container">
        <div className="section-header">
          ...
          <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="btn-secondary-outline btn-sm">
            <span>Get the App</span><ArrowRight size={16} />
          </button>
        </div>
            ...
            <motion.div key={svc.id} className="svc-card" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: idx * 0.1 }}>
              ...
              <div className="svc-info">
                ...
                <div className="svc-perks-row">
                  {svc.perks.map((perk, i) => <span key={i} className="perk-chip"><Zap size={11} />{perk}</span>)}
                </div>
      ...
      <style>{`
        ...
        .svc-bg-img { width:100%; height:100%; object-fit:cover; transition:transform .4s ease; opacity:.85; }
